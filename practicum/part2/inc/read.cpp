...
# define SRC_FILE "input.tsv"

// #define RAND_GRAPH
#define FILE_GRAPH
//#define GRID_GRAPH
// #define GRID_LAYOUT
//#define GRID_SPIRAL_LAYOUT
...
int main(int argc, char** argv)
{
    ...
    __foreach_core(group, core)
    {
        lnh_inst.gpc[group][core]
            ->start_async(__event__(delete_graph));
    }


    unsigned int* host2gpc_ext_buffer   [LNH_GROUPS_COUNT]
                                        [LNH_MAX_CORES_IN_GROUP];
    ...
    __foreach_core(group, core)
    {
        host2gpc_ext_buffer[group][core] = 
            (unsigned int*)lnh_inst.gpc[group][core]
                ->external_memory_create_buffer
        ...
        lnh_inst.gpc[group][core]->
            external_memory_sync_to_device
                (0, 3 * sizeof(unsigned int)*messages_count);
    }
    __foreach_core(group, core)
    {
        lnh_inst.gpc[group][core]
            ->start_async(__event__(insert_edges));
    }
    __foreach_core(group, core) {
        long long tmp = lnh_inst.gpc[group][core]
    ...
    printf("Data graph created!\n");
        ...
        __foreach_core(group, core)
        {
                ...
                lnh_inst.gpc[group][core]->mq_send(GRAPH_SIZE_X);
                lnh_inst.gpc[group][core]->mq_send(GRAPH_SIZE_Y);
#endif
#ifdef GRID_SPIRAL_LAYOUT
